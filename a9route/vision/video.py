...
def suggest_route(shots: list[PercentShot], *, timeline: Timeline | None = None,
    ...
    * 选路：**按"选路段"取结果**（用户 2026-09-15 的口径）——
    ...
    # ⚠️⚠️ **0 张截图 ≠ 「这一局没有操作」**（2026-09-24 用户真踩过）：
    #    一张百分点截图都没有，说明**「路程 NN%」一个都没读到**，
    #    这是"扫描本身失败"，不是"视频里没操作"。以前这里照样打印
    #    「这一局模型没判出任何操作 —— 所以正文是空的（**不是**「没扫描成功」）」——
    #    那句话**恰好说反了** ✗✗：用户拿着这个文件去查按键/阈值，而真凶在读百分比那一步。
    #    现在：空截图就明确说"扫描失败"，并且**原因写进文件**（见 `analysis._with_blocker`）。
    if not shots:
        return "\n".join([
            "# ⚠️ 这次分析**一张百分点截图都没扫到** —— 所以没有路线。",
            "#    这不是「这一局没有操作」，而是**扫描本身失败了**（「路程 NN%」读不到）。",
            "#    原因见命令行输出 / 界面上的红框（这次分析一定带一条 blocker）。",
            "",
            "# （没有可核对的明细 —— 一张截图都没有）",
        ]) + "\n"
    lines = [
